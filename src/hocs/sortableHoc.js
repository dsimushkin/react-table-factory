...
const defaultSortFactory = (parameter, direction) => (a, b) => {
  ...
  let result;
  if (is.string(a[parameter])) {
    result =
      (a[parameter] || "").toUpperCase() >= (b[parameter] || "").toUpperCase();
  } else {
...
export const controlledSortableContext = (
  ...
  forwardRef(
    (
      { data = [], onSort, sortDirection = "asc", sortParameter, ...props },
      ref
    ) => {
      const sortedData = !inMemory
        ? data
        : useMemo(() => {
            const sortedData =
              sortParameter == null
...
export const sortableContext = (
  ...
  return forwardRef(
    (
      { onSort, defaultSortParameter, defaultSortDirection = "asc", ...props },
      ref
    ) => {
